...
public abstract class BaseFormViewModel : BaseObservableObject, Interfaces.IFormViewModel
{
	...
	{
		CloseCommand = new BaseCommand(parameter => Close(SystemControllerEnums.ResultType.No));
		...
		SaveCommand = new BaseCommand(parameter => ExecuteAction(nameof(SaveCommand), parameter),
